...
class feed:
    ...
    def last_link_date(self):
        last_link = db.session.query(Link).order_by(desc(Link.created_at)).limit(1)
        return self.format_datetime(last_link[0].created_at)

    def topic_link(self, link):
        return '<a href="http://credibility.cc/topic/'+str(link.topic.id)+'">'+link.topic.title+' '+link.topic.hashtag+'</a>'
    ...
    def feed_posts(self):
        ...
        for article in articles:
            ...
            post['body'] = 'More articles on '+self.topic_link(article)+' available at '+self.root_link()
            post['category'] = ' > '.join([str(tag.name) for tag in article.topic.tags])
