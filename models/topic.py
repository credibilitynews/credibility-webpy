...
class Topic(Base):
    ...
    def _get_points(self):
        ...
        t = time.mktime(datetime.now().timetuple()) - time.mktime(self.created_at.timetuple())
        p = object_session(self).query(distinct(TopicVote.user_id)).filter_by(topic_id=self.id).count()
    ...
    def _get_fact_stories(self):
        stories =  object_session(self).query(Link).filter_by(topic_id=self.id, type=0).all()
        return  sorted(stories, key=lambda story: story.points, reverse=True)
